...
def main(args):
    ...
    df_tags = pd.read_hdf('./multi_gpu_infer/data/split.hdf','part'+str(args.split_part))
    df_tags = df_tags.head(30)
    print('part shape is ',df_tags.shape)#
    print('=================>loading source data done!')
    text = df_tags.content.values
    all_text=[]
    all_len=[]
    for i in text:
        all_text.append(i)
    ...
    pickle.dump(df_tags,open('./multi_gpu_infer/cmd_based/output/multi_gpu_ljj_range_'+str(args.start)+'_'+str(args.end)+'.pickle','wb'))
